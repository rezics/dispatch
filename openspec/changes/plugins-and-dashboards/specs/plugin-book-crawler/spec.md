...
The plugin config SHALL accept a `rateLimit` field (number, default 10) controlling maximum requests per second across all sources.

#### Scenario: Custom rate limit
- **WHEN** `rateLimit: 5` is configured
- **THEN** the plugin makes at most 5 HTTP requests per second

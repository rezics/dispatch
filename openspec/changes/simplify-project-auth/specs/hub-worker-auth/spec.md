...
### Requirement: Worker authentication via JWT (original)
**Reason**: Replaced by project-scoped JWKS verification. Global auth providers and access policy resolution are no longer used.
**Migration**: Configure `jwksUri` on each project. Workers specify their target project in requests.

### Requirement: WorkerIdentity type (original)
**Reason**: Replaced by simplified WorkerIdentity with single `project` instead of `projects: string[] | '*'`.
**Migration**: Update code that reads `identity.projects` to use `identity.project`.

### Requirement: Project access check on worker routes (original)
**Reason**: Replaced by direct project match check instead of array/wildcard membership.
**Migration**: Replace `requireProjectAccess(identity, projectId)` with direct equality check.

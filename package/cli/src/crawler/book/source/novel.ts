...
export async function fetchNovel(url: string, proxy?: string): Promise<string> {
...
export async function crawlNovel(url: string, proxy?: string): Promise<BookMetadata> {
  const html = await fetchNovel(url, proxy)

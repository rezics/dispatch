...
export async function fetchQidian(url: string, proxy?: string): Promise<string> {
...
export async function crawlQidian(url: string, proxy?: string): Promise<BookMetadata> {
  const html = await fetchQidian(url, proxy)
